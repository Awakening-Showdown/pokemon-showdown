...
pp: 20,
priority: -6,
flags: {reflectable: 1, mirror: 1, sound: 1, mystery: 1},
forceSwitch: true,
secondary: null,
target: "normal",
type: "Cosmic",
...
pp: 10,
priority: 0,
flags: {protect: 1, mirror: 1, distance: 1, sound: 1},
secondary: null,
target: "any",
type: "Fire",
contestType: "Cool",
...
pp: 15,
priority: 0,
flags: {contact: 1, protect: 1, mirror: 1, slicing: 1, wind: 1},
critRatio: 2,
secondary: null,
target: "normal",
type: "Flying",
...
onHit(target, source, move) {
	...
	for (const targetCondition of removeTarget) {
		if (target.side.removeSideCondition(targetCondition)) {
			if (!removeAll.includes(targetCondition)) continue;
			this.add('-sideend', target.side, this.dex.conditions.get(targetCondition).name, '[from] move: Engulf', '[of] ' + source);
			success = true;
		}
	}
	for (const sideCondition of removeAll) {
		if (source.side.removeSideCondition(sideCondition)) {
			this.add('-sideend', source.side, this.dex.conditions.get(sideCondition).name, '[from] move: Engulf', '[of] ' + source);
			success = true;
		}
	}
	this.field.clearTerrain();
